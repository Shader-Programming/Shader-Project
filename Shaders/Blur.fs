...
float weights[21] = float[] (0.016763484915651723, 0.01753817278195756, 0.01830827190172033, 0.019070115902322094, 0.0198199372299882,
...
void main(){
	...
	if(horz == 1){
		for(int i = 1;i < r;i++){
			color = color + texture(image,uv+vec2(textureOffset.x*i,0.0)).rgb*weights[i];
			color = color + texture(image,uv-vec2(textureOffset.x*i,0.0)).rgb*weights[i];
		}
	}else{
		for(int i = 1; i < r;i++){
			color = color + texture(image,uv+vec2(textureOffset.y*i,0.0)).rgb*weights[i];
			color = color + texture(image,uv-vec2(textureOffset.y*i,0.0)).rgb*weights[i];
		}
	}
